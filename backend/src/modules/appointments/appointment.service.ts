...
const allowedTransitions: Record<AppointmentStatus, AppointmentStatus[]> = {
  ...
  CONFIRMADO: ["EM_PÁTIO", "ATRASADO", "CANCELADO"],
  ATRASADO: ["EM_PÁTIO", "CANCELADO"],
  EM_PÁTIO: ["CONCLUÍDO"],
  CONCLUÍDO: [],
  CANCELADO: [],
};

function scheduleStart(item: Pick<Appointment, "scheduledDate" | "scheduledTime">) {
  return new Date(`${item.scheduledDate}T${item.scheduledTime}:00`);
}

function overlaps(first: Appointment, input: NewAppointment) {
  const firstStart = scheduleStart(first).getTime();
  const firstEnd = firstStart + first.estimatedMinutes * 60_000;
  const nextStart = scheduleStart(input).getTime();
  const nextEnd = nextStart + input.estimatedMinutes * 60_000;
  return firstStart < nextEnd && nextStart < firstEnd;
}
...
export class AppointmentService {
  ...
  create(input: NewAppointment) {
    const driver = registryRepository.findDriver(input.driverId);
    const vehicle = registryRepository.findVehicle(input.vehicleId);
    const terminal = registryRepository.findTerminal(input.terminalId);
    ...
    const start = scheduleStart(input);
    if (start.getTime() <= Date.now()) throw new AppError(422, "A janela deve ser agendada para uma data futura");
    ...
    const active = appointmentRepository.list().filter((item) => !["CANCELADO", "CONCLUÍDO"].includes(item.status));
    ...
    const terminalBookings = active.filter((item) => item.terminalId === input.terminalId && item.scheduledDate === input.scheduledDate && item.scheduledTime.startsWith(hour)).length;
    if (terminalBookings >= terminal.hourlyCapacity) throw new AppError(409, "Capacidade horária do terminal atingida");

    return appointmentRepository.create(createId("PA"), input);
  }

  updateStatus(id: string, status: AppointmentStatus) {
    const appointment = this.findById(id);
    if (!allowedTransitions[appointment.status].includes(status)) throw new AppError(422, `Transição de ${appointment.status} para ${status} não permitida`);
