...
export function RegistryPageHeader({ eyebrow, title, description, icon: Icon, count, activeCount, activeLabel, formOpen, onToggleForm }: RegistryPageHeaderProps) {
  return (
    <>
      <div className="registry-heading">
        <div><span>{eyebrow}</span><h2>{title}</h2><p>{description}</p></div>
        <button className={formOpen ? "registry-add-button is-cancel" : "registry-add-button"} type="button" onClick={onToggleForm}>
