...
export function AppointmentsTable({ appointments, onStatusChange, onClearFilters, readOnly = false }: AppointmentsTableProps) {
  if (appointments.length === 0) {
    return <div className="appointments-empty"><span><Container size={28} /></span><h3>Nenhum agendamento encontrado</h3><p>Altere os filtros para consultar outra janela operacional.</p><button type="button" onClick={onClearFilters}>Limpar filtros</button></div>;
  }
  ...
  return (
    <div className="appointments-table-scroll">
      <table className="management-table">
        ...
        <thead><tr><th>Agendamento</th><th>Transportadora</th><th>Motorista e veículo</th><th>Terminal</th><th>Operação</th><th>Status</th>{!readOnly && <th><span className="sr-only">Ações</span></th>}</tr></thead>
