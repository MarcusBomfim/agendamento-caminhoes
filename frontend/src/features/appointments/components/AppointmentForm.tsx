...
import { carrierOptions, gateOptions } from "../data";
...
export function AppointmentForm() {
  ...
  const { register, handleSubmit, formState: { errors, isSubmitting } } = useForm<AppointmentFormValues>({
    ...
    defaultValues: { scheduledDate: today, scheduledTime: "08:00", estimatedMinutes: "45", carrier: "", driverId: "", vehicleId: "", terminalId: "", gate: "", operation: "IMPORTAÇÃO", containerNumber: "", notes: "" },
  });
  ...
  return (
    <form className="appointment-form" onSubmit={handleSubmit(submit)} noValidate>
      <section className="form-section"><header><span><CalendarCheck2 size={20} /></span><div><h3>Janela de atendimento</h3><p>Defina quando e onde o caminhão será recebido.</p></div></header><div className="form-grid">
        ...
        <label className="form-field"><span>Terminal *</span><select {...register("terminalId")}><option value="">Selecione</option>{terminals.filter((item) => item.status !== "INATIVO").map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select><FieldError message={errors.terminalId?.message} /></label>
      ...
      <section className="form-section"><header><span className="section-number">02</span><div><h3>Transporte</h3><p>Vincule a empresa, o motorista e o veículo.</p></div></header><div className="form-grid">
        ...
        <label className="form-field"><span>Motorista *</span><select {...register("driverId")}><option value="">Selecione</option>{drivers.filter((item) => item.status === "ATIVO").map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select><FieldError message={errors.driverId?.message} /></label>
        <label className="form-field"><span>Veículo *</span><select {...register("vehicleId")}><option value="">Selecione</option>{vehicles.filter((item) => item.status !== "INATIVO" && item.status !== "MANUTENÇÃO").map((item) => <option key={item.id} value={item.id}>{item.plate}</option>)}</select><FieldError message={errors.vehicleId?.message} /></label>
